...
contract('ExerciseC6D', async (accounts) => {
  ...
  it('can register oracles', async () => {
    ...
    for(let a=1; a<TEST_ORACLES_COUNT; a++) {      
      ...
      console.log(`Oracle: ${result[0].toNumber()}, ${result[1].toNumber()}, ${result[2].toNumber()}`)
    }
  });
  ...
  it('can request flight status', async () => {
    ...
    await config.exerciseC6D.fetchFlightStatus(flight, timestamp); //Like a user triggering it
